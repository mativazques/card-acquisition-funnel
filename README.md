...
**▶ Live demo:** https://funnel-cockpit-flfedvwjnq-uc.a.run.app
_(Cloud Run, scale-to-zero — the first request after a cold start takes a few seconds to
wake the container.)_
...
Captured from the [live cockpit](https://funnel-cockpit-flfedvwjnq-uc.a.run.app).
...
**Deploy shape:** **two** scale-to-zero Cloud Run services (`min-instances=0`) — the
Streamlit cockpit (the public demo link) and the copilot API (the `/ask` backend the
cockpit calls). They are separate images because Streamlit (protobuf<6) and the ADK agent
layer (protobuf 6.x) cannot share one environment. Terraform owns only the serving layer;
the data layer is left as bootstrapped so IaC cannot destroy loaded data.
...
- A free [Kaggle account](https://www.kaggle.com) → *Settings → API → Create New Token*.
  Kaggle now issues a **bearer token** (`KGAT_…`); the ingest script reads it from
  `KAGGLE_API_TOKEN` in `.env` or from `~/.kaggle/access_token`.
...
The stack needs **two** isolated venvs because of a conflicting protobuf pin (Streamlit
requires protobuf<6, `google-adk` requires 6.x — they cannot share an environment, and by
extension cannot share a container image):

```bash
# .venv (Python 3.9) — dbt + Streamlit cockpit + the semantic package
...
```bash
make tf-init          # terraform init (serving layer)
make tf-bootstrap     # Artifact Registry + secret container + enable APIs
make secret-push      # push GEMINI_API_KEY into Secret Manager (value stays out of git/tf)
make image-push       # build linux/amd64 + push BOTH images (cockpit + copilot-api)
make deploy           # terraform apply the two Cloud Run services; prints both URLs
make trim             # drop raw GCS + BQ raw table (keep marts)
make teardown         # destroy serving layer (data layer kept)
```

### MCP server (local stdio — not deployed)

The same four governed tools (`list_metrics`, `query_metric`, `compare_cohorts`,
`explain_metric`) are also exposed over MCP for use in an MCP client (Claude Desktop,
Cursor). It runs locally over stdio — it is **not** part of the Cloud Run deploy. Point a
client at it with:
...
```json
{
  "mcpServers": {
    "card-funnel": {
      "command": "/absolute/path/to/card-acquisition-funnel/.venv-agents/bin/python",
      "args": ["-m", "agents.mcp_server"],
      "cwd": "/absolute/path/to/card-acquisition-funnel",
      "env": { "GCP_PROJECT": "your-project", "BQ_DBT_DATASET": "analytics" }
    }
  }
}
```

Run it standalone with `make mcp`. BigQuery auth is via ADC.
